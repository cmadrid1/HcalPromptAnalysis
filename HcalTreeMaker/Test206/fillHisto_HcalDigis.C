...
void fillHisto_HcalDigis(const char *infile, const char *outfile, const char *flag = "single", double min_totcharge_HPD  = 0 , double max_totcharge_HPD  = 100 ){

  double min_totcharge_SiPM  = 5000; //32000; //5000fC (~2GeV)
  //double min_totcharge_HPD = 0; //600;  //Not Sure where to put this
  //double max_totcharge_HPD = 100;
  double min_totcharge_HF    = 200; //3200;
  ...
  TTreeReaderValue<Int_t> bx(*tReader, "bx");
  ...
  while(tReader->Next()){
	...
	if(totsumHBHE > min_totcharge_HPD && totsumHBHE < max_totcharge_HPD){
